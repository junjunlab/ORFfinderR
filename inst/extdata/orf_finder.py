    ...
    for start_codon in all_starts:
        ...
        for stop_codon in all_stops:
            ...
            if right_frame and right_sense and length > 0:
                ...
                # Registered ORF
                if right_length:
                    orfs.append({"start": position, "end": stop_codon["position"], "frame": frame, "sense": sense, "length": length, "trailing": False,"id": id, "t_len": total_len})
        ...
        if warp_case and not trim_trailing:
            ...
            if right_length:
                orfs.append({"start": position, "end": -1, "frame": frame, "sense": sense, "length": length, "trailing": True,"id": id, "t_len": total_len})
...
def getORFNucleotides (sequence="", return_loci=False, **kwargs):
    ...
    sequence_fa = SeqIO.read(sequence, "fasta")
    sequence = __reformSequence(sequence_fa)
...
def getORFProteins (sequence, translation_table=1, **kwargs):
